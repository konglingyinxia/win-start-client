<template>
  ...
  <el-drawer
      ...
      @opened="addEventListenerLog(data)"
      :size="elDrawerSize"
      @closed="handleClose"
  >
    <template #title>
...
const handleClose = () => {
  logApi.CloseAppLog(data.value.id).then((res: any) => {
    if (res.code === 200) {
...
const addEventListenerLog = (params: any) => {
  //读取日志
  logApi.AppLog(params.id).then((res: any) => {
    if (res.code === 200) {
      EventsOnName.value = params.id;
...
const acceptParams = (params: any) => {
  ...
  titleData.value = params.name;
  if (params.remark) {
    titleData.value += `(${params.remark})`;
  }
}
